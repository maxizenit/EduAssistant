...
VALUES ('Добрый день! Когда дедлайн по заданию №2?', dateadd(hour, -3, NOW()), 6, 1, 1),
       ('Здравствуйте! В понедельник', dateadd(hour, -2, NOW()), 1, 6, 1),
       ('А если не успеваю?', dateadd(hour, -1, NOW()), 6, 1, 1),
       ('В каком формате сдавать проект?', dateadd(hour, -2, NOW()), 7, 3, 2),
       ('Задание сдаем через GitHub', dateadd(hour, -3, NOW()), 3, 7, 2),
       ('Спасибо большое!', dateadd(hour, -1, NOW()), 7, 3, 2),
       ('Какие материалы подготовить к следующей паре?', dateadd(hour, -4, NOW()), 8, 4, 3),
       ('Зависит от вашего проекта', dateadd(hour, -2, NOW()), 4, 8, 3),
       ('Понял', dateadd(hour, -6, NOW()), 8, 4, 3),
       ('Как проходят тесты?', dateadd(hour, -3, NOW()), 10, 3, 4),
       ('Я их запускаю локально', dateadd(hour, -1, NOW()), 3, 10, 4),
       ('Как запустить проект?', dateadd(hour, -7, NOW()), 11, 1, 5),
       ('Посмотрите в описании репозитория', dateadd(hour, -5, NOW()), 1, 11, 5);
