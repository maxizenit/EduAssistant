...
@BotState
class StudentInQuestionViewState(
...
) : State(stateSwitcher, keyboardCreator, messageUtils, mapOf(Command.BACK to StudentInQuestionMenuState::class)) {

    private final val currentQuestionIdDataType = DataType.STUDENT_IN_QUESTION_VIEW_STATE_CURRENT_QUESTION_ID
    ...
    override fun handleCallback(callback: CallbackQuery): BotApiMethod<*> {
        ...
        dataCache.putInputDataValue(chatId, currentQuestionIdDataType, channelId)
        return initState(chatId)
    }
}